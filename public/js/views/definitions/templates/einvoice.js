$(document).ready(function () {
    ...
    const invoice_table = $("#einvoice-templates-table").DataTable({
        language: {
        ...
        ordering: false,
        ajax: {
            url: "/datatablesQuery/definitions/templates/1",
        },
        columns: [
            { data: "id" },
            { data: "name" },
    ...
    $("#einvoice-save-from-file").click((e) => {
        let file = $("#einvoice-template-file").prop("files")[0];
        const templateName = $("#einvoice-template-name").val();
            ...
            .then((result) => {
                ...
                if (response.status) {
                    Swal.fire({
                        ...
                        $("#einvoice-template-file").val("");
                        $("#einvoice-template-name").val("");
                        invoice_table.draw();
                        $("#einvoice-fromfile").modal("hide");
                    });
                } else {
                    Swal.fire(
                        "Hata!!",
    ...
    $("#einvoice-templates-table tbody").on(
        "click",
        "#make-default",
        function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "PUT",
                        url: `/definitions/templates/1/${data.id}/default`,
        ...
        function () {
            ...
            Swal.fire({
                ...
                if (result.isConfirmed) {
                    $.ajax({
                        type: "PUT",
                        url: `/definitions/templates/1/${data.id}/status`,
                        success: function (response) {
                            invoice_table.draw();
                            toastr["success"](
